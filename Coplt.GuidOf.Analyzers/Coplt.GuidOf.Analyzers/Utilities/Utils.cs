...
internal static class Utils
{
    ...
    public static string GetGenerics(this INamedTypeSymbol symbol, bool arg = false)
    {
        var generic = string.Empty;
        if (symbol.IsGenericType)
        {
    ...
    public static string GetWheres(this INamedTypeSymbol symbol)
    {
        ...
        if (symbol.IsGenericType)
        {
            ...
            foreach (var tp in symbol.TypeParameters)
            {
                ...
                if (tp.HasReferenceTypeConstraint) items.Add("class");
                if (tp is { HasValueTypeConstraint: true, HasUnmanagedTypeConstraint: false }) items.Add("struct");
                if (tp.HasUnmanagedTypeConstraint) items.Add("unmanaged");
                if (tp.HasNotNullConstraint) items.Add("notnull");
                items.AddRange(tp.ConstraintTypes.Select(static t => t.ToDisplayString()));
                if (items.Count == 0) continue;
                if (tp.HasConstructorConstraint) items.Add("new()");
                ps.Add($"where {tp.ToDisplayString()} : {string.Join(", ", items)}");
            }
            if (ps.Count == 0) return "";
            wheres = $" {string.Join(" ", ps)}";
        }
        return wheres;
    }
